    ...
    var methods = {
        ...
        _createStars: function () {
            for (var i = 1; i <= this.opt.number; i++) {
                ...
                $('<' + this.opt.starType + ' />', attrs).attr({
                    width: '16',
                    height: '16',
                    srcset: attrs.src + ' 1x, ' + attrs.src.replace(".png", "-2x.png/index.html") + ' 2x'
                }).appendTo(this);
                if (this.opt.space) {
        ...
        _setIcon: function (star, icon) {
            ...
            star['srcset'] = this.opt.path + icon + ' 1x, ' + this.opt.path + icon.replace(".png", "-2x.png/index.html") + ' 2x';
        },
...
if (matchMedia_support) {
    var max_767 = window.matchMedia('(max-width: 767px)').matches;
}

function ListChapProcess(truyen_name, truyen_ascii, page, page_list, chap_list) {
    $('#list-chapter .pagination').replaceWith(page_list);
    $('#list-chapter>.row').html(chap_list);
    $('html, body').animate({
    ...
    window.history.pushState({
        ...
        "truyen_ascii": truyen_ascii,
        "page": page
    }, "Truyện " + truyen_name + " - Trang " + page, "/" + truyen_ascii + "/trang-" + page + "/#list-chapter");
    document.title = "Truyện " + truyen_name + " - Trang " + page;
    ga('send', 'pageview', {
        'page': '/' + truyen_ascii + '/trang-' + page + '/',
        'title': "Truyện " + truyen_name + " - Trang " + page + ' - Ajax'
    });
}
$(document).ready(function () {
    if (!$.browser.mobile) {
        ...
        if (!has3d) $("#truyen").removeClass('csstransforms3d');
    }
    if ($(".list-same-author").is(":hidden")) {
        $(".list-same-author").detach().appendTo("#truyen").addClass("bottom-similar");
    }
    if ($(".list-similar").is(":hidden")) {
        $(".list-similar").detach().appendTo("#truyen").addClass("bottom-similar");
    }
    if ($("body").is("#body_truyen")) {
        update_views(truyenid, 'truyen');
    }
    var fb_loaded = 0;
    var truyencomment;
    $(window).scroll(function () {
        if (truyenid && fb_loaded == 0) {
            ...
            setTimeout(function () {
                load_comment('fb-comment-story', truyencomment, fbcolor);
            }, 2000);
            fb_loaded = 1;
        }
    });
    $('#list-chapter').on('submit', '#page_jump', function (e) {
        e.preventDefault();
        var cur = $("#page_jump").find("button[type='submit']");
        var truyen_name = $('h1').text();
        var totalp = $('#total-page').val();
        ...
        if (isNumber(page) && (page % 1) === 0) {
            ...
            $.get("../ajax.html", {
                ...
                ListChapProcess(truyen_name, truyen_ascii, page, data.page_list, data.chap_list);
                cur.css({
                    "cursor": "pointer",
                    "pointer-events": "auto",
                    "opacity": "1"
    ...
    $('#list-chapter').on('click', '.pagination li a', function (e) {
        ...
        if (href != 'javascript:void(0)') {
            ...
            var truyen_name = $('h1').text();
            var totalp = $('#total-page').val();
            if (href.indexOf(truyen_ascii + '/#list-chapter') !== -1) {
                page = 1;
            } else {
                var page = href.match(/(\d+)(?!.*\d)/);
                page = page[1];
            }
            $.get("../ajax.html", {
                type: "list_chapter",
    ...
    if ($('.col-truyen-side').is(":visible")) {
        $.get("../ajax.html", {
            type: "top_switch",
            data_type: 'day'
        }).done(function (data) {
            var html_top_side = '<div class="title-list"><h2>Truyện đang hot</h2></div><div class="row top-nav" data-limit="10"><div class="col-xs-4 active" data-type="day">Ngày</div><div class="col-xs-4" data-type="month">Tháng</div><div class="col-xs-4" data-type="all">All time</div></div>';
            html_top_side += data;
            $('#top-list-side-container').replaceWith(html_top_side);
        });
    }
    $('.list-side').on('click', '.top-nav div', function () {
        var cur = $(this);
        var data_type = cur.attr('data-type');
        var data_limit = cur.parent().attr('data-limit');
    ...
    $('.rate-holder').raty({
        number: 10,
        path: '//static.trumtruyen.vip/img/raty',
        noRatedMsg: "Đã đánh giá",
        hints: ['Không còn gì để nói...', 'WTF', 'Cái gì thế này ?!', 'Haizz', 'Tạm', 'Cũng được', 'Khá đấy', 'Được', 'Hay', 'Tuyệt đỉnh!'],
        target: '.rate-text',
        score: function () {
            return $(this).attr('data-score');
        },
        click: function (score) {
            $('.rate-holder').raty('readOnly', true);
            $.post("../ajax.html", {
                type: "rate",
                data: score,
                id: truyenid
            });
            localStorage.setItem(truyenid, 'rated');
            alert('Cảm ơn bạn đã đánh giá truyện!');
        },
        readOnly: function () {
            var check_rate = localStorage.getItem(truyenid);
            if (check_rate == 'rated') return true;
        }
    });
    var chaptersac = $("#chapter-sac").val();
    gtag('event', 'truyen_views');
    if (chaptersac == 1) gtag('event', 'truyen_views_sac');
});;
